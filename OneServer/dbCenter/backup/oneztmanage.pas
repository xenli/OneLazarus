...
type
  ...
  public
    ...
    function OpenDataBySQL(QZTCode: string; QSql: string; QParams: array of variant; var QErrMsg: string): TFDMemtable;
  public
...
begin
  ...
  //stoUseImplicit：表示使用隐式事务。当设置为 stoUseImplicit 时，
  //TSQLTransaction 组件将自动为每个执行的 SQL 语句创建一个事务，并在执行完成后自动提交。这是默认的事务选项。

  //stoExplicitStart：表示使用显式事务。当设置为 stoExplicitStart 时，
  //TSQLTransaction 组件需要手动调用 StartTransaction 方法来开始一个事务，并在完成后调用 Commit 或 Rollback 方法来提交或回滚事务。
  self.Options := [];
...
begin
  ...
  FDQuery.InsertSQL.Clear;
  FDQuery.DeleteSQL.Clear;
  FDQuery.UpdateSQL.Clear;
  FDQuery.SQL.Text := '';
...
begin
  ...
  try
    ...
    try
      ...
      Result.CopyFromDataset(LZTQuery, True);
    except
      on e: Exception do
      begin
...
function TOneZTManage.OpenDataBySQL(QZTCode: string; QSql: string; QParams: array of variant; var QErrMsg: string): TFDMemtable;
var
  lZTItem: TOneZTItem;
  LZTQuery: TFDQuery;
  iParam: integer;
begin
  ...
  begin
    if QErrMsg = '' then
      QErrMsg := '获取账套[' + QZTCode + ']连接失败,原因未知';
    exit;
  end;
  try
    // 打开数据屏B删除,插入,删除功能防止打开数据的SQL里面放有执行DML语句
    LZTQuery := lZTItem.GetQuery;
    LZTQuery.SQL.Text := QSql;
    // 参数赋值
    if LZTQuery.Params.Count <> length(QParams) then
    begin
      QErrMsg := 'SQL最终参数个数和传进的参数个数不一至';
      ...
      try
        ...
        begin
          ...
          begin
            // 提交数据
            LZTQuery := lZTItem.ADQuery;
            LZTQuery.SQL.Text := 'select *  from ' + lDataSaveDML.TableName + ' where 1=2 ';
