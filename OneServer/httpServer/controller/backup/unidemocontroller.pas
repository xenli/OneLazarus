...
type
  ...
  IUniDemoController = interface
    ...
    function GetGoodsList(pageIndex: integer; pageSize: integer; goodInfo: string): TActionResult<TBufDataSet>;
...
function TUniDemoController.DoInvoke(QRttiMethod: TRttiMethod; const aArgs: array of TValue): TValue;
var
...
begin
 {$if defined(CPUI386) or (defined(CPUX86_64) and defined(WIN64))}
  ...
  begin
    Result := self.GetGoodsList(aArgs[0].AsInteger, aArgs[1].AsInteger, aArgs[2].AsString);
  end
  else
...
function TUniDemoController.GetGoodsList(pageIndex: integer; pageSize: integer; goodInfo: string): TActionResult<TBufDataSet>;
var
  lZTItem: TOneZTItem;
  lFDQuery: TFDQuery;
  lOneTokenItem: TOneTokenItem;
    ...
    try
      ...
      begin
        ...
        Lsql :=
          ...
          ' where FGoodsCode like :goodInfo or FGoodsName like :goodInfo ' + #13#10 + ' order by FGoodsCode ';
        //独立的一行Order by 设置分页时分析用到
      end;
...
// 返回一个商品类表,跟据传上来的Json数据过滤相关数据
function TUniDemoController.GetGoodsListByJson(QJson: TJsonObject): TActionResult<TBufDataSet>;
var
  lZTItem: TOneZTItem;
  lFDQuery: TFDQuery;
  lOneTokenItem: TOneTokenItem;
...
begin
  ...
  lGoodDemo := TGoodsDemo.Create();
  lGoodDemo.FGoodsID := '123';
  lGoodDemo.FGoodsCode := '123';
  lGoodDemo.FGoodsName := '123';
  lGoodDemo.FGoodsPrice := 123;
...
begin
  ...
  begin
    ...
    Result.resultData := Result.resultData + '当前接收到文件参数[' + lWebRequestFile.FieldName + ']' +
      '文件名称[' + lWebRequestFile.fileName + ']' + #10#13;
    // 文件流 ,至于要咱样是业务问题
