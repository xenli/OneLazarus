...
uses
  Classes, SysUtils, OneILog, OneThread, generics.Defaults,
  Generics.Collections, SyncObjs, OneFileHelper;
...
type
  ...
  private
    ...
    FCSFileNameLock: TCriticalSection;
  ...
  public
    ...
    property LogListenLevel: TELogLevel read FLogListenLevel write FLogListenLevel;
    property LogIntervalLevel: TELogIntervalLevel read FLogIntervalLevel write FLogIntervalLevel;
...
begin
  ...
  FCSFileNameLock := TCriticalSection.Create;
  FCurFileName := '';
  FFileName := '';
  FLogPath := QLogPath;
  FLogInterval := 1;
...
begin
  ...
  try
    ...
    begin
      SourceString := FormatDateTime('yyyy-mm-dd hh:mm:ss zzz', now()) + '->' + QMsg + #13#10;
      bytes := TEncoding.UTF8.GetBytes(SourceString);
      lBytesStream := TBytesStream.Create(bytes);
      try
        lBytesStream.Position := 0;
        FLogBuff.CopyFrom(lBytesStream, lBytesStream.Size);
      finally
        lBytesStream.Clear;
        lBytesStream.Free;
      end;
...
begin
  ...
  begin
    FLogPath := OneFileHelper.FormatPath(Self.FLogSet.LogPath);
  end;
  FLogPath := OneFileHelper.CombinePath(FLogPath, 'OneLogs');
  ...
  FLogItemLock := TCriticalSection.Create;
end;
